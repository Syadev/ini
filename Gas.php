...
for ($ia=1; $ia <= $totalReff; $ia++) {
    ...
    $data = '{"email":"'.$email.'","password":"Syadev22","phone_number":"852'.$nomorHP.'"}';
    ...
    if ($token) {
        ...
        if ($profileCard) {
            ...
            if ($anuSukses) {
                echo "[ ".date('H:i:s')." ] -> ".$colors->getColoredString("[$codereff] EKSEKUSI BERHASIL", "green").PHP_EOL;
            } else {
                echo "[ ".date('H:i:s')." ] -> FAIL ".$colors->getColoredString("1", "red").PHP_EOL;
                goto tryRegister;
            }
        } else {
            echo "[ ".date('H:i:s')." ] -> FAIL ".$colors->getColoredString("2", "red").PHP_EOL;
            goto tryRegister;
        }
    } else if($error === "Kamu terkena batasan") {
        echo "[ ".date('H:i:s')." ] -> LIMIT ".$colors->getColoredString("CUY", "red").PHP_EOL;
        goto tryRegister;
    } else {
        echo "[ ".date('H:i:s')." ] -> FAIL ".$colors->getColoredString("DUH", "red").PHP_EOL;
	goto tryRegister;
    }
...
class Colors {
    ...
    public function __construct() {
        ...
        $this->foreground_colors['yellow'] = '1; lol33';
